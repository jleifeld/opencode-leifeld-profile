...
function resolveSessionID(requestedSessionID: string | undefined, contextSessionID: string | undefined) {
  const requested = requestedSessionID?.trim()
  if (requested && requested !== "current") return requested

  const contextID = contextSessionID?.trim()
...
async function loadTiktokenEncoder(model: string) {
  ...
  const encodingForModel = mod.encodingForModel ?? mod.encoding_for_model
  const getEncoding = mod.getEncoding ?? mod.get_encoding
  let encoder
  try {
...
async function importFromVendor(pkg: string) {
  ...
  try {
    ...
    throw new Error(
      "Context usage dependencies missing. Run npm install js-tiktoken@latest @huggingface/transformers@^3.3.3 --prefix .opencode/plugins/vendor to install vendor tokenizers.",
    )
  }
...
function formatTokenizerResolutionError(error: TokenizerResolutionError, sessionID: string): string {
  ...
  lines.push("Install or update vendor tokenizers by running npm install js-tiktoken@latest @huggingface/transformers@^3.3.3 --prefix .opencode/plugins/vendor.")
